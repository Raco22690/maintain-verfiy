...
try {
    ...
    if (!empty($attch)) {
        ...
        if (strpos($attch, 'temp_') === 0) {
            ...
            $newPath = __DIR__ . '/attachments/' . $id . '.' . pathinfo($attch, PATHINFO_EXTENSION);
            if (file_exists($oldPath)) {
                if (!rename($oldPath, $newPath)) {
                    throw new Exception('重命名附件失敗');
                }
                $attch = $id . '.' . pathinfo($attch, PATHINFO_EXTENSION);
            }
        }
    }

    // 準備 SQL 語句
    $sql = "UPDATE BKM_table SET 
            Device = ?, 
            SWBin_Issue = ?, 
            Sort = ?, 
            Description = ?, 
    ...
    $stmt->bind_param('ssssssssss', 
        $device, $swbin, $sort, $desc, 
        $attch, $status, $site, $updater, 
        $last_update, $id
    );
