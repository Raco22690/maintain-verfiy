...
.bkm-table th:nth-child(11), .bkm-table td:nth-child(11) { width: 3%; min-width: 10px; }
        ...
        #modalAttch {
            ...
            font-family: inherit;
            font-size: 14px;
            border: 1px solid rgb(210, 188, 140);
            border-radius: 4px;
        ...
        #verifyModal .verify-table-header {
            display: grid;
            grid-template-columns: 150px 1fr 120px 2fr 40px; /* Type | Target | OD | Remark | Action */
            gap: 0;
            background: #6c757d;
            color: white;
            font-weight: 600;
        ...
        #verifyModal .verify-table-row {
            display: grid;
            grid-template-columns: 150px 1fr 120px 2fr 40px; /* Match header */
            gap: 0;
            align-items: stretch;
            margin-bottom: 0;
            background: #f8f9fa;
...
<main class="main-container">
    ...
    <div class="tab-bar">
        <div class="tab-container">
            ...
            <a class="tab-button active" href="../V65_Maintian verify/V67_Maintian verify A2.php">Device Verify information</a>
...
function addVerifyDataItem(data = null) {
    ...
    const newItemHtml = `
        <div class="dynamic-verify-item" id="verify-item-${itemIndex}">
            <button type="button" class="delete-item-btn" onclick="this.parentElement.remove()">×</button>
            <div class="form-row">
                <label>Type</label>
                <input type="text" name="verify_data[${itemIndex}][type_name]" placeholder="e.g., Sort1 點測" value="${data?.type_name || ''}">
            </div>
            <div class="form-row" data-columns="2">
                <label>Test Program</label>
                <input type="text" name="verify_data[${itemIndex}][test_program]" value="${data?.test_program || ''}">
                <label>Test OD</label>
                <input type="text" name="verify_data[${itemIndex}][test_od]" value="${data?.test_od || ''}">
            </div>
            <div class="form-row" data-columns="2">
                <label>Prober file</label>
                <input type="text" name="verify_data[${itemIndex}][prober_file]" value="${data?.prober_file || ''}">
                <label>Clean OD</label>
                <input type="text" name="verify_data[${itemIndex}][clean_od]" value="${data?.clean_od || ''}">
            </div>
        </div>
    `;
...
function removeVerifyDataItem(itemId) {
    const item = document.getElementById(`verify-item-${itemId}`);
    if (item) {
        item.remove();
        
        // 如果刪除後沒有項目了，也移除表格標題
        const remainingItems = document.querySelectorAll('#verify-data-section .dynamic-verify-item');
        if (remainingItems.length === 0) {
            const header = document.querySelector('#verify-data-section .verify-table-header');
            if (header) {
                header.remove();
            }
        }
...
function openVerifyModal(data = null) {
    ...
    // Clear dynamic verify data items
    ...
    if (data) {
        ...
        // Populate dynamic verify items
        if (data.verify_data && Array.isArray(data.verify_data)) {
            data.verify_data.forEach(item => {
                addVerifyDataItem(item);
            });
        }
    } else {
...
function submitVerifyForm(event) {
    ...
    // Manually collect dynamic verify data
    data.verify_data = [];
    const verifyItems = document.querySelectorAll('#verify-data-section .dynamic-verify-item');
    verifyItems.forEach((item, index) => {
        const typeName = item.querySelector(`input[name="verify_data[${index}][type_name]"]`).value;
        const testProgram = item.querySelector(`input[name="verify_data[${index}][test_program]"]`).value;
        const testOd = item.querySelector(`input[name="verify_data[${index}][test_od]"]`).value;
        const proberFile = item.querySelector(`input[name="verify_data[${index}][prober_file]"]`).value;
        const cleanOd = item.querySelector(`input[name="verify_data[${index}][clean_od]"]`).value;

        // Only add item if at least one field has a value
        if (typeName || testProgram || testOd || proberFile || cleanOd) {
    ...
    // You would typically send 'data' to your server via fetch/AJAX
    ...
    setTimeout(() => {
        closeVerifyModal();
        showToast('資料已成功儲存！');
        // Here you would also refresh the list, e.g., searchVerifyTable()
    }, 500);
}
...
function searchVerifyTable() {
    ...
    // After search, you would call a function like renderVerifyList(results)
}

function cleanVerifySearch() {
    document.getElementById('verifySearchInput').value = '';
    // Placeholder to show all items
    console.log("Clearing search...");
}

// ... more functions for rendering list, preview, etc. would go here ...
