...
.bkm-table th:nth-child(11), .bkm-table td:nth-child(11) { width: 3%; min-width: 10px; }
        ...
        #verifyModal .form-row {
            display: grid;
            grid-template-columns: auto 1fr; /* Default: Label + Input */
            gap: 10px 15px;
            align-items: center;
            margin-bottom: 10px;
        }
        /* Use data-columns attribute to create multi-column rows */
        /* e.g., data-columns="2" creates 2 pairs of Label+Input */
        ...
        #verifyModal .dynamic-verify-item .form-row {
            grid-template-columns: auto 1fr;
        }
        #verifyModal .dynamic-verify-item .form-row[data-columns="2"] {
            grid-template-columns: auto 1fr auto 1fr;
        }
...
function switchTab(tabName) {
    // ... (Tab switching code remains) ...
    if (tabName === 'Verify') {
        // Any specific logic for when Verify tab is clicked
    }
}
    ...
    const newItemHtml = `
        <div class="dynamic-verify-item" id="verify-item-${verifyItemCounter}">
            <div class="item-form-fields">
                <div class="form-row" data-columns="2">
                    <div>
                        <label for="verify_type_${verifyItemCounter}">Verify Type</label>
                        <select id="verify_type_${verifyItemCounter}" name="verify_type[]"></select>
                    </div>
                    <div>
                        <label for="target_${verifyItemCounter}">Target</label>
                        <input type="text" id="target_${verifyItemCounter}" name="target[]">
                    </div>
                </div>
                <div class="form-row">
                     <label for="od_spec_${verifyItemCounter}">OD Spec</label>
                     <input type="text" id="od_spec_${verifyItemCounter}" name="od_spec[]">
                </div>
                 <div class="form-row">
                    <label for="remark_${verifyItemCounter}">Remark</label>
                    <textarea id="remark_${verifyItemCounter}" name="remark[]"></textarea>
                </div>
            </div>
            <button type="button" class="delete-item-btn" onclick="removeVerifyDataItem(${verifyItemCounter})">&times;</button>
        </div>
    `;
    ...
    if (data) {
        ...
        if (data.verify_data && Array.isArray(data.verify_data)) {
            data.verify_data.forEach(item => {
                addVerifyDataItem(); // This increments the counter
                document.getElementById(`verify_type_${verifyItemCounter}`).value = item.verify_type;
                document.getElementById(`target_${verifyItemCounter}`).value = item.target;
                document.getElementById(`od_spec_${verifyItemCounter}`).value = item.od_spec;
                document.getElementById(`remark_${verifyItemCounter}`).value = item.remark;
            });
        }
    } else {
...
function submitVerifyForm(event) {
    ...
    verifyItems.forEach((item, index) => {
        data.verify_data.push({
            verify_type: item.querySelector('select[name="verify_type[]"]').value,
            target: item.querySelector('input[name="target[]"]').value,
            od_spec: item.querySelector('input[name="od_spec[]"]').value,
